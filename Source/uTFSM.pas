...
begin
  if FileExists(ini_serverconfig) then
    begin
      ini := TIniFile.Create(ini_serverconfig);
      try
        steamport := ini.ReadString('Server Config', 'serverSteamPort', steamport);
        gameport := ini.ReadString('Server Config', 'serverGamePort', gameport);
        queryport := ini.ReadString('Server Config', 'serverQueryPort', queryport);
        servername := ini.ReadString('Server Config', 'serverName', servername);
        maxplayers := ini.ReadString('Server Config', 'serverPlayers', maxplayers);
        vac := ini.ReadString('Server Config', 'enableVAC', vac);
        serverpass := ini.ReadString('Server Config', 'serverPassword', serverpass);
        adminpass := ini.ReadString('Server Config', 'serverPasswordAdmin', adminpass);
        steamtoken := ini.ReadString('Server Config', 'serverSteamAccount', steamtoken);
        autosave := ini.ReadString('Server Config', 'serverAutoSaveInterval', autosave);
        difficulty := ini.ReadString('Server Config', 'difficulty', difficulty);
        inittype := ini.ReadString('Server Config', 'initType', inittype);
        saveslot := ini.ReadString('Server Config', 'slot', saveslot);
        showlog := ini.ReadString('Server Config', 'showLogs', showlog);
        email := ini.ReadString('Server Config', 'serverContact', email);
        ...
        TStyleManager.TrySetStyle(ini.ReadString('Application Settings', 'Theme', ''))
      finally
        ini.Free;
      end;
    end;
